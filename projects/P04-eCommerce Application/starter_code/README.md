...
To create a new ecommerceUser for example, you would send a POST request to:
http://localhost:8080/api/ecommerceUser/create with an example body like 
...
Exercise:
Once you've created a ecommerceUser, try  to add items to cart (see the `ModifyCartRequest` class) and submit an order. 
...
2. Create password for the ecommerceUser
3. Once that is disabled, you will need to implement 4 classes (at minimum, you can break it down however you like):
   ...
   * an implementation of the `UserDetailsService` interface. This should take a username and return a userdetails User instance with the ecommerceUser's username and hashed password.
   *  a subclass of `WebSecurityConfigurerAdapter`. This should attach your ecommerceUser details service implementation to Spring's `AuthenticationManager`. It also handles session management and what endpoints are secured. For us, we manage the session so session management should be disabled. Your filters should be added to the authentication chain and every endpoint but 1 should have security required. The one that should not is the one responsible for creating new users.
